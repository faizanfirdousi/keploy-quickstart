...
import recordGif from '@/components/keploy-record.gif';

export function RecordTerminalGif() {
  return (
    <figure className="not-prose my-8">
      <div className="overflow-hidden rounded-xl border border-zinc-200/80 bg-[#121215] shadow-xl dark:border-white/10 dark:shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-zinc-200/80 bg-zinc-100/90 px-4 py-2.5 dark:border-white/[0.08] dark:bg-zinc-900/80">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 font-mono text-xs text-zinc-600 dark:text-zinc-400">
              terminal — keploy record &amp; traffic session
            </span>
          </div>
        ...
        <div className="relative bg-[#0c0c0e]">
          <Image
            src={recordGif}
            alt="Terminal recording of Keploy record mode and curl traffic generation"
            unoptimized
      ...
      <figcaption className="mt-3 text-center text-sm text-muted-foreground">
        Live terminal recording: Keploy instrumentation (left) capturing requests and PostgreSQL queries as curl commands run (right).
      </figcaption>
    </figure>
  );
}
